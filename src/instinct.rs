...
impl DriveLevel {
    ...
    #[must_use] #[inline]
    pub fn is_critical(&self) -> bool { self.0 > 0.8 }
    #[must_use] #[inline]
    pub fn is_sated(&self) -> bool { self.0 < 0.2 }

    pub fn increase(&mut self, amount: f32) { self.0 = (self.0 + amount).clamp(0.0, 1.0); }
    pub fn decrease(&mut self, amount: f32) { self.0 = (self.0 - amount).clamp(0.0, 1.0); }
}
...
impl Instinct {
    ...
    pub fn new(instinct_type: InstinctType) -> Self {
        Self { instinct_type, drive: DriveLevel::new(0.5), priority: 0.5 }
    }

    /// Update priority based on drive level and time since last satisfied.
    pub fn update_priority(&mut self) {
        self.priority = self.drive.value() * match self.instinct_type {
...
pub fn dominant_instinct(instincts: &[Instinct]) -> Option<&Instinct> {
    instincts.iter().max_by(|a, b| a.priority.partial_cmp(&b.priority).unwrap_or(std::cmp::Ordering::Equal))
}
