...
#[must_use]
pub fn select_threat_response(aggression: f32, speed: f32, relative_size: f32, social_rank: f32) -> ThreatResponse {
    ...
    let scores = [(fight_score, ThreatResponse::Fight), (flight_score, ThreatResponse::Flight),
                  (freeze_score, ThreatResponse::Freeze), (fawn_score, ThreatResponse::Fawn)];
    scores.iter().max_by(|a, b| a.0.partial_cmp(&b.0).unwrap_or(std::cmp::Ordering::Equal)).unwrap().1
}
